    ...
    async with websockets.connect(qwen_ws, max_size=10*1024*1024) as ws:
        ...
        while True:
            ...
            (() => {
                ...
                const msgs = document.querySelectorAll('.qwen-chat-message-assistant .custom-qwen-markdown, .chat-response-message .custom-qwen-markdown');
                let lastText = "";
                if (msgs.length > 0) {
            ...
            if elapsed > 900: # 15 minutes rule
                print("[-] 15-minute timeout reached.")
                break
