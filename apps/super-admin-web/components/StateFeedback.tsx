  ...
  return (
    <div className="flex flex-col items-center justify-center p-12 text-center space-y-3 bg-slate-950/40 rounded-2xl border border-dashed border-slate-800 my-4">
      ...
      {actionText && onAction && (
        <button
          onClick={onAction}
          className="mt-2 px-4 py-2 bg-purple-600 hover:bg-purple-500 text-white rounded-xl text-xs font-bold transition-all shadow-lg shadow-purple-900/30"
        >
          {actionText}
        </button>
      )}
...
export function ErrorBanner({
  message = "خطایی در دریافت داده‌های راهبری رخ داده است.",
  onRetry
}: {
  message?: string;
  onRetry?: () => void;
