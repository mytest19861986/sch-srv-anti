...
const DRIVER_ICON_192 = fs.readFileSync(path.join(__dirname, 'public', 'icons', 'icon-192x192.png'));
const DRIVER_ICON_512 = fs.readFileSync(path.join(__dirname, 'public', 'icons', 'icon-512x512.png'));

fastify.get('/icons/icon-192x192.png', async (req, reply) => {
  reply.header('Content-Type', 'image/png').send(DRIVER_ICON_192);
});

fastify.get('/icons/icon-512x512.png', async (req, reply) => {
  reply.header('Content-Type', 'image/png').send(DRIVER_ICON_512);
});
...
<body class="text-slate-100 min-h-screen flex flex-col justify-between p-4 max-w-md mx-auto">
  ...
  <div id="pwa-install-banner" class="mb-3 glass p-3 rounded-2xl border border-indigo-500/40 flex items-center justify-between shadow-xl">
    <div class="flex items-center gap-2.5">
      <span class="text-xl">📲</span>
      <div>
        <p class="text-xs font-bold text-white">نصب اپلیکیشن راننده روی گوشی</p>
        <p class="text-[10px] text-indigo-300/80">دسترسی سریع به مانیفست و کارکرد آفلاین</p>
      </div>
    </div>
    <button id="btn-pwa-install" onclick="triggerPwaInstall()" class="px-3 py-1.5 rounded-xl bg-indigo-500 hover:bg-indigo-400 text-white text-xs font-black shadow-md transition-all active:scale-95">
      نصب اپ
    </button>
  </div>
  ...
  <div id="modal-pwa-guide" class="hidden fixed inset-0 z-50 bg-black/80 backdrop-blur-md flex items-center justify-center p-4">
    <div class="glass p-6 rounded-3xl max-w-sm w-full space-y-4 border border-indigo-500/40 shadow-2xl">
      <div class="text-center space-y-2">
        <span class="text-3xl">📲</span>
        <h3 class="text-base font-black text-white">راهنمای نصب نسخه راننده</h3>
        <p class="text-xs text-slate-300 leading-relaxed">
          برای نصب مستقیم اپلیکیشن سرویس‌یار راننده روی صفحه اصلی گوشی:
        </p>
      </div>
      <div class="bg-slate-900/80 p-3.5 rounded-2xl space-y-2 border border-indigo-900/40 text-xs text-indigo-200">
      ...
      <button onclick="closePwaGuide()" class="w-full py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-bold transition-all">
        متوجه شدم
      </button>
    </div>
  </div>
      ...
      try {
        ...
        container.innerHTML = \`
          <div class="glass p-5 rounded-3xl text-center space-y-3 border border-rose-500/30 shadow-lg">
            ...
            <button onclick="fetchDriverManifest()" class="px-4 py-2 rounded-xl bg-amber-600 hover:bg-amber-500 text-white text-xs font-bold transition-all shadow-md">
              🔄 تلاش مجدد
            </button>
          </div>
        \`;
