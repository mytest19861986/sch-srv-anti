...
export async function POST(req: NextRequest) {
  try {
    ...
    // Role check for School Dashboard: SCHOOL_ADMIN or SCHOOL_OPERATOR
    const allowedRoles = ["SCHOOL_ADMIN", "SCHOOL_OPERATOR"];
    if (!allowedRoles.includes(data.user?.role)) {
      return NextResponse.json(
        { error: "دسترسی غیرمجاز: این پنل صرفاً متعلق به مدیران مدرسه است." },
        { status: 403 }
      );
    }
    ...
    response.cookies.set("tenant_id", data.user?.tenantId || "", {
      httpOnly: true,
      secure: false,
      sameSite: "lax",
      path: "/",
